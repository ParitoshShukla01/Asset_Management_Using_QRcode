...
public class MroLogin extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.mrologin);
        mContext = MroLogin.this;
                ...
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            Query phone_numberQuery=FirebaseDatabase.getInstance().getReference().child("employee").orderByChild("empid").equalTo(phone_number.substring(3));
                            phone_numberQuery.addListenerForSingleValueEvent(new ValueEventListener() {
                                ...
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    if(dataSnapshot.getChildrenCount()>0) {
                                        ...
                                        FirebaseDatabase.getInstance().getReference().child("employee").child(phone_number.substring(3)).child("verify")
                                                .get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                                            @Override
                                            public void onComplete(@NonNull Task<DataSnapshot> task) {
                                                String check="false";
                                                if(task.isSuccessful())
                                                    check =String.valueOf(task.getResult().getValue());
                                                Log.e("check", "onComplete: "+ String.valueOf(task.getResult().getValue()));

                                                if(check.equals("true"))
                                                {
                                                    Intent i = new Intent(MroLogin.this, scan.class);
                                                    startActivity(i);
                                                    finish();
                                                }
                                                else
                                                {
                                                    Toast.makeText(mContext, "Profile not verified yet !", Toast.LENGTH_LONG).show();
                                                    mAuth=FirebaseAuth.getInstance();
                                                    mAuth.signOut();
                                                }
                                            }
                                    ...
                                    {
                                        Intent intent=new Intent(MroLogin.this,NewJoinee.class);
                                        intent.putExtra("phno",phone_number.substring(3));
